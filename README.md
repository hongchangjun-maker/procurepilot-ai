...
- `ADMIN_PASSWORD`: 초기값 `6085`. 운영 전 긴 고유 비밀번호로 변경 필수
...
공공데이터포털의 `조달청_나라장터 입찰공고정보서비스` 활용신청 후 서비스키를 `DATA_GO_KR_SERVICE_KEY`에 설정합니다. 현재 MVP는 용역 목록 오퍼레이션을 사용합니다. 물품·공사·사전규격·발주계획은 `lib/connectors.ts`의 동일 인터페이스로 추가할 수 있습니다.

### 기업마당

기업마당 정책정보 개방에서 지원사업정보 API 사용신청 후 인증키를 `BIZINFO_API_KEY`에 설정합니다.

robots.txt, 이용약관, 저작권, 재배포 조건을 기관별로 검토해야 합니다. HTML 수집 커넥터는 요청 간격, 타임아웃, 식별 가능한 User-Agent, 재시도 상한, 원문 링크 보존을 구현한 뒤 활성화하세요. 인증·접근 제한을 우회하지 마세요.

## OpenAI 연결

OpenAI 키는 배포 환경변수로만 등록합니다. 관리자 화면의 ‘연결 테스트’는 서버가 키를 보유하는지만 확인합니다. 분석은 Responses API의 `json_schema` 구조화 출력으로 수행하며 결과를 D1에 저장합니다.
...
상단 `마스터 관리자` → 초기 비밀번호 `6085`.

초기 비밀번호는 개발 편의를 위한 값입니다. 운영 전 `ADMIN_PASSWORD`와 `APP_ENCRYPTION_KEY`를 변경하고, 향후에는 사용자 계정, MFA, 역할 기반 권한, 감사 로그, 로그인 속도 제한을 추가해야 합니다.
...
스크래핑 커넥터는 기관별 선택자와 허용 정책이 다르므로 관리자 입력값을 곧바로 실행하지 말고, 서버의 허용된 커넥터 레지스트리와 검증 스키마를 거치도록 확장하세요.
